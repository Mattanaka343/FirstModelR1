---
title: Primer Acercamiento a un Modelo
...
El propósito de este documento es presentar el primer acercamiento al entrenamiento de un modelo para los datos que nos fueron dados

## El Problema

El problema a resolver para este reto es uno de clasificación. En este se busca determinar que actividad esta siendo realizada por una persona dados los valores medidos por sensores de IMU que el infividuo usa en uno de sus brazos, una de sus piernas y su mano. Los valores retornados por estos sensores se refieren a aceleraciones y velocidades angulares. 

## Instrucciones de la Actividad

La actividad nos pide realizar una extracción de datos y posteriormente una transformación que los haga aptos para  su consumo en un modelo. Después de esta manipulación se nos pide que implementemos algún algoritmo de machine learning de nuestra preferenecia sin hacer uso de librerías para clasificar los datos obtenidos a partir de los sensores. Finalmente se nos pide que probemos la interpretación. Aunádo a todo nos pide que el modelo en si se guarde en un archivo de compilado separado de un notebook, llámese particularmente en mi caso (Dado que trabajo con python) con un `.py`

## Los Datos y su Preprocesamiento 

Como se mencionó antes los datos con los que estamos tratando representan una señales. Trabajar con las señales como si fuesen *features* no es particularmente funcional, es por esto que llevamos un proceso de discrtización. Para esta discretización elegí ciertas transformaciones que podrían de manera fácil representar la señal. Estas se aplicarán a toda la señal para evitar terminar con features irregulares y señales sobrepresentadas o alta multicolinealidad.  Además se verificará que los features diseñados para este ejercicio no esten altamente correlacionados para evitar problemas. Los features que seleccionamos a primer principio fueron los siguientes:

1. **Media de la señal**: Nos da una idea de alrededor de dónde oscila.
2. **Desviación Estándard de la Señal**: Nos da una idea sobre la amplitud general de la misma.
3. **Componentes de La Rápida de Fourier**: Nos da una idea de las cosas que componene la señal
...
df.to_csv('../Data/TransformedData.csv',index = False)
```

## Modelo 

El modelo de machine learning que elegí implementar fue k-nearest neighbors debido a su facilidad de uso y al hecho de que es fácil de comprender. 
...
Con este modelo se obtuvo un valor de alrededor de 0.84 lo cuál es bastante bueno para esta implenmentación debido a que las clases están mayormente balanceadas. El modelo que se usó fue KNNClassifier con un parámetro k de 10 y usando la distancia euclidiana. 